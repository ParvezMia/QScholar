<x-app-layout>
    <div class="bg-indigo-600 px-8 pt-10 lg:pt-14 pb-16 flex justify-between items-center mb-3">
        <!-- title -->
        <h1 class="text-xl text-white">Edit Organization</h1>
    </div>
                ...
                <form action="{{ route('organizations.update', $organization->id) }}" method="post"
                    ...
                    <div class="py-3">
                        ...
                        <input type="text" id="name" name="name"
                            ...
                            placeholder="Name" value="{{ old('name', $organization->org_name) }}" />
                        @error('name')
                    ...
                    <div class="grid lg:grid-cols-2 gap-4">
                        <div class="py-3">
                            ...
                            <input type="email" id="email" name="email"
                                class="border border-gray-300 text-gray-900 rounded focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2 px-3"
                                placeholder="Email" value="{{ old('email', $organization->org_email) }}" />
                            @error('email')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="py-3">
                            ...
                            <input type="url" id="website" name="website"
                                class="border border-gray-300 text-gray-900 rounded focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2 px-3"
                                placeholder="Website" value="{{ old('website', $organization->org_website) }}" />
                            @error('website')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="py-3">
                        ...
                        <input type="text" id="contact_number" name="contact_number"
                            ...
                            value="{{ old('contact_number', $organization->org_contact_number) }}" />
                    ...
                    <div class="py-3">
                        ...
                        <textarea id="address" name="address"
                            class="border border-gray-300 text-gray-900 rounded focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2 px-3"
                            placeholder="Address">{{ old('address', $organization->org_address) }}</textarea>
                        @error('address')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="py-3 mt-5">
                        <button type="submit"
                            class="bg-indigo-600 text-white rounded py-2 px-4 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-500">Update</button>
                    </div>
                </form>
            </div>
        </div>
